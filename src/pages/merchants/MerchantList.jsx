...
export default function MerchantList() {
  ...
  // Active filter states (only applied on Search click)
  const [activeFilters, setActiveFilters] = useState({
    merchantId: "",
    storeName: "",
    status: "",
  ...
  // ✅ FIX BUG 4: Send merchantId and storeName as separate params matching backend
  const handleSubmitFilters = () => {
    setActiveFilters({
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    queryFn: async () => {
      const params = { page, limit };
      // ✅ FIX BUG 4: Send correct param names to backend
  ...
  // ✅ FIX BUG 1: Response now includes isWithdrawalForbidden — invalidate to refetch
  const toggleWithdrawal = useMutation({
    mutationFn: (m) => API.put(`/merchants/${m._id}/withdrawal-status`),
    onSuccess: (res) => {
      invalidate(); // refetch so button text updates
      toast.success(
  ...
  return (
    ...
    >
              ...
              {isLoading ? (
                ...
                merchants.map((m) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-5 text-center">
                      {/* ✅ FIX BUG 1: Use correct field m.status (not m.isActive) */}
                      <span
                        style={{ padding: "5px" }}
                    ...
                    <td className="py-3 px-5">
                      <div className="flex items-center justify-center gap-2 w-max mx-auto opacity-90 group-hover:opacity-100 transition-opacity">
                        ...
                        {(isSuperAdmin || isMerchantAdmin) && (
                          // ✅ FIX BUG 1: Use correct field isWithdrawalForbidden
                          <ActionBtn
                            color={
                        ...
                        {/* ✅ FIX BUG 3: Bank Card shows single bankCard + bankName fields */}
                        <ActionBtn
                          color="#475569"
                          label="Bank Card"
                          onClick={() => openModal("bankcard", m)}
      ...
      {/* 5. Bank Card Modal */}
      {/* ✅ FIX BUG 3: Show bankCard + bankName fields from merchant model (not bankCards array) */}
      <Modal
        open={modal === "bankcard"}
        onClose={closeModal}
